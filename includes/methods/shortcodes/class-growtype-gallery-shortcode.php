...
class Growtype_Gallery_Shortcode
{
    ...
    function growtype_gallery_shortcode($atts)
    {
        ...
        if (empty($parent_id)) {
            $parent_id = 'growtype-gallery-' . $post_type;
        }
        ...
        if ($post_type === 'multisite_sites') {
            ...
            $args = apply_filters('growtype_gallery_shortcode_extend_args', $atts, $args);
    ...
    public static function render_all(
        ...
        if (!empty($posts)) : ?>
            <div <?php echo !empty($parent_id) ? 'id="' . $parent_id . '"' : "" ?> class="growtype-gallery-container <?php echo $parent_class ?> <?php echo $slider === 'true' ? 'b-posts-slider' : '' ?>" data-columns="<?php echo $columns ?>">
                <?php
    ...
    public static function render_single(
        ...
        return growtype_gallery_include_view($template_path,
            [
                'post' => $post,
